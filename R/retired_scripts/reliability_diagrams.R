...
rd <- sc %>% 
  ...
  filter(!is.na(observation),
         variable=="temperature", # all grouping variables
         horizon==7,   
         depth==10)
...
df <- expand.grid(variable = c('temperature', 'oxygen'),
                depth = c(1, 10),
                horizon = c(1, 7, 35),
                bin = seq(10, 90, by = 10),
                obs_freq = NA,
...
folders <- "all_UC_fsed_deep_DA"




out <- plyr::ddply(df, c("variable", "depth", "horizon", "bin"), \(x){
  print(head(x))
  temp <- rd
  vec <- c()
    ...
    for(i in 1:nrow(temp)){
      ...
      prob_above <- qnorm(x$high[j], temp$mean[i], temp$sd[i])
      prob_below <- qnorm(x$low[j], temp$mean[i], temp$sd[i])
    ...
    x$obs_freq[j] <- sum(vec)/nrow(temp)*100
    vec <- c()
...
ggplot(dat, aes(x = bin, y = bin, color = UC)) +
  ...
  ggtitle('temperature 10m horizon = 1')
